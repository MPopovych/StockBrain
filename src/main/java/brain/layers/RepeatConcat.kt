...
class RepeatConcat(
	...
	init {
		if (parentLayers.size != 2 || parentHeight.size != 2) {
			throw IllegalStateException("Illegal count/heights of parents: ${parentHeight}}")
		}
	}
	...
	override fun create(): RepeatConcatImpl {
		return RepeatConcatImpl(concatShape, name).also {
			it.init()
		}
	}
...
class RepeatConcatImpl(
	private val concatShape: LayerShape,
